...
SceneDirector.register('task-route', function(container, manifest, reduced) {
  ...
  var EXAMPLES = [
    ...
    {
      label: 'Route a structured filing',
      sub: null,
      dims: { rule: 80, input: 78, ambig: 12, action: 42, control: 45 },
      layer: 1,
      layerColor: '#55C7E8'
    },
    {
  ...
  function build() {
    ...
    _taskIdx.textContent = '01 OF 0' + EXAMPLES.length;
  ...
  function _setExample(idx) {
    var ex = EXAMPLES[idx];
    _taskIdx.textContent = '0' + (idx + 1) + ' OF 0' + EXAMPLES.length;
